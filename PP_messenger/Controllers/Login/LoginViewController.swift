...
final class LoginViewController: UIViewController {
    ...
    @objc private func loginButtonTapped() {
        ...
        FirebaseAuth.Auth.auth().signIn(withEmail: email, password: password, completion: { [weak self] authResult, err in
            ...
            print("logged in with email: \(user)")
...
extension LoginViewController: LoginButtonDelegate {
    ...
    func loginButton(_ loginButton: FBLoginButton, didCompleteWith result: LoginManagerLoginResult?, error: Error?) {
        ...
        facebookRequest.start(completionHandler: { _, result, error in
            ...
            FirebaseAuth.Auth.auth().signIn(with: credential, completion: { [weak self] authResult, err in
                guard let strongSelf = self else {
                    return
                }
                guard authResult != nil, err == nil else {
                    if let err = err {
                        print("error, cannot sign in with facebook - \(err)")
                    }
                    return
                }
                strongSelf.navigationController?.dismiss(animated: true, completion: nil)
